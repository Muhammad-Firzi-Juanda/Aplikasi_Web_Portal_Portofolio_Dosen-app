...
const USER_SERVICE_URL = process.env.USER_SERVICE_URL || 'http://api-gateway:80';
...
class AuthController {
  ...
  async login(req, res, next) {
    try {
      ...
      try {
        const profileResponse = await axios.get(`${USER_SERVICE_URL}/api/users/${user.id}`);
  ...
  async changePassword(req, res, next) {
    try {
      ...
      // Update password and clear refresh token to force re-login
      await user.update({ 
  ...
  // Internal endpoint: Delete user (called from user-service)
  async deleteUserInternal(req, res, next) {
    try {
      const { id } = req.params;
